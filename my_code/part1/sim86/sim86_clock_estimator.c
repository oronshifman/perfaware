...
clock_est_t EstimatorGetClocks(expression_t *decoded_inst)
{
    ...
    clock_est_t clock_estimation = clock_estimation_table[op_type][dest_access][src_access];

    // TODO(16.7.24): turn this if into a table
    if (clock_estimation.ea_clocks == UNINITIALIZED)
    {
        ...
        {
            clock_estimation.ea_clocks = 6;
        }
        else if (mem_operand->ea_code >= 4)
        {
            if (mem_operand->disp == 0)
            {
                clock_estimation.ea_clocks = 5;
            }
            else
            {
                clock_estimation.ea_clocks = 9;
            }
        }
        else if (mem_operand->ea_code == 0 || mem_operand->ea_code == 3)
        {
            if (mem_operand->disp == 0)
            {
                clock_estimation.ea_clocks = 7;
            }
            else
            {
                clock_estimation.ea_clocks = 11;
            }
        }
        else if (mem_operand->ea_code == 2 || mem_operand->ea_code == 1)
        {
            if (mem_operand->disp == 0)
            {
                clock_estimation.ea_clocks = 8;
            }
            else
            {
                clock_estimation.ea_clocks = 12;
            }
        }
    }
